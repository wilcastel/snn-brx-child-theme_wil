...
// Configuration
$update_proxy_url = 'https://sinanisler.com/snn-brx-wil-github/update.php';
    ...
    <script type="text/javascript">
        (function() {
            ...
            function modifyLink(link) {
                if (link.getAttribute('aria-label') && link.getAttribute('aria-label').includes('SNN-BRX-WIL')) {
            ...
            function processLinks() {
                const links = document.querySelectorAll('a[aria-label*="SNN-BRX-WIL"]');
            ...
            const observer = new MutationObserver(function(mutations) {
                mutations.forEach(function(mutation) {
                    mutation.addedNodes.forEach(function(node) {
                        if (node.nodeType === 1) {
                            if (node.matches && node.matches('a[aria-label*="SNN-BRX-WIL"]')) {
                                modifyLink(node);
                            }
                            const links = node.querySelectorAll && node.querySelectorAll('a[aria-label*="SNN-BRX-WIL"]');
                            if (links) {
                                links.forEach(modifyLink);
                            }
                        }
